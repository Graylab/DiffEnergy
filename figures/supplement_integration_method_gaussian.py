...
if __name__ == "__main__":
    ...
    # Figure 3
    likelihood_dir = Path('results/likelihood/gaussian_integration_methods')
    ...
    add_row(subfs_grid[2][0],next(labelit),'Trapezoidal Integration',likelihood_dir,Path('../gaussian_1d_diff_trapezoid'),'integrand:TotalIntegrand','prior:smax_gaussian',binline=True)
    ...
    add_row(subfs_grid[3][1],next(labelit),'Flow Trajectory ODE Integration',likelihood_dir,Path('../gaussian_1d_flow'),'integrand:TotalIntegrand','prior:smax_gaussian',binline=True)
    ...
    compare_likelihoods(next(compit),next(labelit),"Trapezoidal\nvs Interpolated",likelihood_dir/'../gaussian_1d_diff_trapezoid','Trapezoidal',likelihood_dir/'gaussian_1d_diff_interpolated','Interpolated Trapezoidal')
    compare_likelihoods(next(compit),next(labelit),"Trapezoidal\nvs Piecewise ODE",likelihood_dir/'..'/'gaussian_1d_diff_trapezoid','Trapezoidal',likelihood_dir/'gaussian_1d_diff_piecewise_ode','Piecewise ODE')
