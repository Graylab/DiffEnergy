# --------------------------------------------------------------------------
# author Sudeep Sarma
# sample from a 2D diffusion model
# --------------------------------------------------------------------------
import torch
import functools
...
def Euler_Maruyama_sampler(score_model,
    ...
    init_x = torch.randn(batch_size, dimension, device=device) * marginal_prob_std(t)[:, None]
...
# ----------------------------------------------------------------------------------
# Main
@hydra.main(version_base=None, config_path="../configs", config_name="sample_gaussian_2d")
def main(config: DictConfig):
    ...
    if tr_type == 'non_conservative':
        score = ScoreNetMLP(input_dim = 2, marginal_prob_std = marginal_prob_std_fn, embed_dim = 512, layers = (512, 512, 512)).to(device)
    elif tr_type == 'conservative':
        score = NegativeGradientMLP(input_dim = 2, marginal_prob_std = marginal_prob_std_fn, embed_dim = 512, layers = (512, 512, 512)).to(device)
