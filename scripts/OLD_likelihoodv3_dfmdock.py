...
import functools
import shutil
from typing import Callable, Collection, Generic, Iterable, Literal, Mapping, Optional, ParamSpec, Protocol, Sequence, Sized, TypeVar, TypeVarTuple, overload
...
class MapDataset(Generic[X,*P], Dataset[X]):
    def __init__(self, source:Sequence[tuple[*P]], map:Callable[[*P],X]) -> None:
        self.source = source
        self.map = map
...
@overload
def load_samples(data_file, data_dir, offset_type: Literal["Translation", "Rotation", "Translation+Rotation"], importer:PDBImporter, batch_size:None,device:str|torch.device='cuda')->Collection[tuple[str,torch.Tensor,DFMDict]]|DataLoader: ...
@overload
def load_samples(data_file, data_dir, offset_type: Literal["Translation", "Rotation", "Translation+Rotation"], importer:PDBImporter, batch_size:int,device:str|torch.device='cuda')->Collection[tuple[Sequence[str],torch.Tensor,DFMDict]]|DataLoader: ...
def load_samples(data_file, data_dir, offset_type: Literal["Translation", "Rotation", "Translation+Rotation"], importer:PDBImporter, batch_size:int|None=None, device:str|torch.device='cuda')->Collection[tuple[str,torch.Tensor,DFMDict]]|Sequence[tuple[Sequence[str],torch.Tensor,DFMDict]]|DataLoader:
    """Loads dataset from a CSV file and returns an iterable of tuples ('id', x, condition).
    if batch_size is None, each x will be an array of shape D. Otherwise, x has shape BxD for batch_size B
    """
    ...
    def getpdb(id:str,path:Path)->tuple[str,torch.Tensor,DFMDict]:
        datum = importer.get_pdb(str(path),id)
        dfmdict:DFMDict = {
        ...
        return (id,
                torch.zeros((6 if offset_type == "Translation+Rotation" else 3,),device=device,dtype=datum["lig_pos"].dtype),
                dfmdict)
    
    data_dir = Path(data_dir)
    
    return DataLoader(MapDataset([(str(id),data_dir/path) for id,path in zip(ids,paths)],getpdb))
...
def main(config: DictConfig):
    ...
    match config.path_type:
        case "flow_ode":
            ...
            dataloader = load_samples(config.data_listpdb, data_dir, offset_type, pdb_importer, batch_size=batch_size, device=device)
        ...
        case "data_translation":
            ...
            paths = (
                (id,LinearPath[LigDict,DFMDict]((from_array(start),0),(from_array(end),0),ode_times, #start and end both 0!
                            config.odeint_rtol,
                            config.odeint_atol,
                            config.odeint_method,
                            to_array,
