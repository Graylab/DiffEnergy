...
@overload
def load_samples(data_path, batch_size:None,device:str|torch.device='cuda')->list[tuple[str,torch.Tensor]]: ...
@overload
def load_samples(data_path, batch_size:int,device:str|torch.device='cuda')->list[tuple[Sequence[str],torch.Tensor]]: ...
def load_samples(data_path, batch_size:int|None=None, device:str|torch.device='cuda')->list[tuple[str,torch.Tensor]]|list[tuple[Sequence[str],torch.Tensor]]:
...
def main(config: DictConfig):
    ...
    match config.path_type:
        case "flow_ode":
            ...
            dataloader = load_samples(config.data_samples,batch_size=batch_size, device=device)
