...
from typing import Literal, DefaultDict, Mapping, MutableMapping, Optional
...
def load_neighborhood_stats(samples_source:Literal['centerline_shifts', 'transverse_plane']):
    ...
    if samples_source == 'centerline_shifts':
        sample_folder = "../../neighborhood_sampling/centerline_shifts/"
    elif samples_source == 'transverse_plane':
        sample_folder = "../../neighborhood_sampling/transverse_plane/"
    sample_folder = Path(sample_folder)
    
    sample_stats = get_neighborhood_sample_stats(sample_folder)
    ...
    if samples_source == 'centerline_shifts':
        ...
        sources = {'flow_nll': ('transverse_plane_flow_40',), 'forwardsde_nll': ('transverse_plane_forward_sde',)}
    else:
        raise ValueError()
...
def load_newcode_likelihoods(sources:Mapping[str,str|tuple[str,...]],integrand:str,prior:str,
                             sample_stats:MutableMapping[str,pd.DataFrame],srcfolder:Path=Path(".")):
...
def plot_all_grids(sample_stats:Mapping[str,pd.DataFrame],gt_stats:Mapping[str,dict[str,float]],labels:Mapping[str,str|None],limits:Mapping[str,tuple[float|None,float|None]|None],save:bool=True,outdir:Path|None=None):
    ...
    exclude = ('pdb_id','Filename','sample_index','Basis File','Plane Offset','X','Y')
