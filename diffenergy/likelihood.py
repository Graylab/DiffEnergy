...
class FlowSpaceIntegral:
    ...
    def ode_likelihood(self, batch):
        ...
        def ode_func(t, x):
            ...
            #dlogp(x,t)/dx = F
            dlogp_dx = self.score_eval_wrapper(batch, self.score_model, device = self.device)

            #dx/dt = -1/2g(t)^2*dlogp/dx
            sample_grad = -0.5 * g**2 * dlogp_dx

            #F dot dx
            dEnergy = torch.dot(dlogp_dx,sample_grad)

            # logp_grad = -0.5 * g**2 * self.divergence_eval_wrapper(batch, self.score_model, device = self.device)
            sample_logp_concat = torch.cat([sample_grad, dEnergy], dim=0)
...
class FlowTotalIntegral:
    ...
    def ode_likelihood(self, batch):
        ...
        def ode_func(t, X):
            ...
            #dlogp(x,t)/dt assuming x indep of t
            dlogp = -0.5 * g**2 * self.divergence_eval_wrapper(batch, self.score_model, device = self.device)
            
            #chain rule: dlogp(x(t),t)/dt = dlogp(x,t)/dt + dlogp(x,t)/dx*dx/dt
            dlogp_dt = dlogp + torch.dot(dlogp_dx,sample_grad)

            #integrate dx/dt, dlogp/dt at the same time
            sample_logp_concat = torch.cat([sample_grad, dlogp_dt], dim=0)
            
            #return [dx/dt,dlogp/dt] as X' 
            return sample_logp_concat
